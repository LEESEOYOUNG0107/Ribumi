...
export default function ReviewSection({ itemReviews, onEditSave, onDelete, onReplySubmit, onLikeToggle }) {
        ...
        <div key={review.id} className="reviewCard">
          <div className="reviewHeader">
            <div className="reviewUser">
              <div className="reviewAvatar">{review.user_id?.[0] || "?"}</div>
              <div>
                <span className="reviewUserName">{review.user_id}</span>
          ...
          {editingReviewId === review.id ? (
            <div style={{ margin: "10px 0" }}>
              ...
              <div className="reviewFormBtns" style={{ justifyContent: "flex-end", marginTop: "10px" }}>
                <button className="reviewCancelBtn" onClick={() => setEditingReviewId(null)}>
                  취소
                </button>
                <button className="reviewSubmitBtn" onClick={() => handleSaveEdit(review.id)}>
                  수정 완료
                </button>
              </div>
            </div>
            ...
            <>
              ...
              <div className="reviewFooter">
                <div style={{ display: "flex", gap: 6 }}>
                  <button className="reviewActionBtn" onClick={() => handleEditClick(review)}>
                    수정
                  </button>
                  <button className="reviewActionBtn" onClick={() => onDelete(review.id)}>
                    삭제
                  </button>
                  <button className="reviewActionBtn" onClick={() => handleReplyClick(review.id)}>
                    답글
                  </button>
                </div>
                <div className="reviewReactions">
                  ...
                  <span>답글 {review.comments || 0}</span>
                </div>
              </div>
                ...
                <div className="replyForm">
                  ...
                  <div className="reviewFormBottom">
                    ...
                    <div className="reviewFormBtns">
                      <button className="reviewCancelBtn" onClick={() => setReplyOpenId(null)}>
                        취소
                      </button>
                      <button className="reviewSubmitBtn" onClick={() => handleSaveReply(review.id)}>
                        등록
                      </button>
                    </div>
                  </div>
